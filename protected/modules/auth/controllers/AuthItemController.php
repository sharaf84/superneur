...
abstract class AuthItemController extends AuthController {
    ...
    public function actionCreate() {
        ...
        $this->render('create', array(
            'model' => $model,
            'operations' => ($model->type == 0) ? $this->getAdminOperations() : array()//used to make auto complete if type = operation @auther Ahmed Sharaf   
        ));
    }
    ...
    /**
     * Gets Admin controllers operations (actions)
     * @author Ahmed Sharaf <sharaf.developer@gmail.com>
     * @return array 
     */
    protected function getAdminOperations() {
        $operations = array();
        $adminControllersActions = Yii::app()->metadata->getControllersActions('admin');
        $parentAdminControllerActions = Yii::app()->metadata->getActions('AdminController', 'components');
        foreach ($adminControllersActions as $controller) {
            foreach ($controller['actions'] as $action) {
                if (in_array($action, $parentAdminControllerActions))
                    break;
                $operation = strtolower('admin.' . substr($controller['name'], 0, -10) . '.' . $action);
