...
class JFlowPlayer extends CWidget {
    ...
    public function init() {
        ...
        $playerUrl = $this->baseUrl . '/flowplayer-3.2.16.swf';

        $cs->registerScriptFile($this->baseUrl . '/flowplayer-3.2.12.min.js');

        $cs->registerScript('flowplayer-script-' . $this->id, 'flowplayer("' . $this->id . '", "' . $playerUrl . '");');
    }

    public function play() {
        return '<a href="' . $this->url . '" id="' . $this->id . '" style="display:block; width:' . $this->width . '; height:' . $this->height . '"></a>';
    }
