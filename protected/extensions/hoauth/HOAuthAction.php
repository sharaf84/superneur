...
class HOAuthAction extends CAction {
    ...
    protected function oAuth($provider) {
        ...
        try {
             ...
             */
            return $this->customAction($oAuth);
    ...
    /**
     * set required data to session then redirect
     * @author Ahmed Sharaf
     * @param obj $oAuth
     */
    protected function customAction($oAuth){
        if(!Yii::app()->user->isGuest || empty($oAuth->profile))
            Yii::app()->controller->redirect(Yii::app()->homeUrl);
        $profile = $oAuth->profile;
        $sessionArr = array();
        $sessionArr['provider'] = $oAuth->provider;
        foreach ($this->_avaibleAtts as $attr)
            $sessionArr['profile'][$attr] = isset($profile->{$attr}) ? $profile->{$attr} : '';
        unset(Yii::app()->session['oAuth']);
        Yii::app()->session['oAuth'] = $sessionArr;
        Yii::app()->controller->redirect(Yii::app()->createUrl('site/socialLogin'));
    }
    
    public static function twitterCustomAction($tokens){
        if(!Yii::app()->user->isGuest)
            Yii::app()->controller->redirect(Yii::app()->homeUrl);
        $sessionArr = array();
        $sessionArr['provider'] = 'Twitter';
        $sessionArr['profile']['identifier'] = isset($tokens['user_id']) ? $tokens['user_id'] : '';
        $sessionArr['profile']['userName'] = isset($tokens['screen_name']) ? $tokens['screen_name'] : '';
        $sessionArr['profile']['firstName'] = $sessionArr['profile']['lastName'] = $sessionArr['profile']['emailVerified'] = '';
        unset(Yii::app()->session['oAuth']);
        Yii::app()->session['oAuth'] = $sessionArr;
        Yii::app()->controller->redirect(Yii::app()->createUrl('site/socialLogin'));
    }
