...
class ETinyMce extends CInputWidget
{
   ...
   *
    ...
    * @var array
    */
   private $switchLabels = array(0=>'Теxt mode', 1=>'HTML mode');
   ...
   protected function makeFullEditor($url='')
   {
      ...
      $this->plugins = array('safari','pagebreak','style','layer','table','save','advhr','advimage','advlink','emotions','spellchecker','inlinepopups','insertdatetime','preview','media','searchreplace','print','paste','directionality','fullscreen','noneditable','visualchars','nonbreaking','xhtmlxtras','template');
      ...
      if ($this->useElFinder) {
          $options['file_browser_callback'] = 'js:function(field_name, url, type, win) {
            ...
            if($("#finder").length == 0) {
                ...
                //$("#finder").elfinder({ zIndex: 9999999, places: "Textmode", url : "'.Yii::app()->urlManager->createUrl('backend/elfinder/default/connector').'",
                ...
                $("#finder").elfinder({ zIndex: 9999999, places: "Textmode", url : "'.Yii::app()->getAssetManager()->publish(dirname(__FILE__).DIRECTORY_SEPARATOR.'assets').'/elfinder/connectors/connector.php?url='.Yii::app()->baseUrl.'/uploads/'.'", 
                    lang : "en",
                    dialog : { width : 900, modal : true, title : "Files", zIndex: 7777777 },
