...
class alpha extends CValidator
{
	 ...
	 */
	private $_basicPattern = '/^{startWithChar}[a-zA-Z{additionalParams}]{{minChars},{maxChars}}$/';
        /**
	 * @var string pattern for start with char.
	 */
	private $_startWithCahr = '[A-z]+';
        /**
	...
	/**
	 * @var boolean whether to start with char or not.
	 */
        public $startWithChar = false;
	/**
...
";
	...
	{
		$additionalParams = $startWithChar = NULL;
		// add numbers
		...
		if (count($this->extra))
			$additionalParams .= implode('\\', $this->extra);
                ...
                //check start with cahr
                if($this->startWithChar){
                    $this->minChars--;
                    $this->maxChars--;
                    $startWithChar = $this->_startWithCahr;  
                };
			
		return str_replace(array('{startWithChar}', '{additionalParams}', '{minChars}', '{maxChars}'), array($startWithChar, $additionalParams, $this->minChars, $this->maxChars), $this->_basicPattern);
	}
}
