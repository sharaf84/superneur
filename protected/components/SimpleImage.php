...
class SimpleImage extends CApplicationComponent {
    ...
    public $name = 'Image';
    private $image, $filename, $original_info, $width, $height, $exif;
...
//    }
     ...
     */
    public function fit_and_crop($max_width, $max_height) {

        // If original dimension < required dimension => do nothing.
        if ($this->width <= $max_width && $this->height <= $max_height){
            return $this;
        }
        // If width or height of (original dimension < required dimension) => Make best fit.  
        elseif($this->width < $max_width || $this->height < $max_height){
            return $this->best_fit($max_width, $max_height);
        }
        
        // Required ratio
        $required_ratio = $max_height / $max_width;
        // Determine aspect ratio
        $aspect_ratio = $this->height / $this->width;
        //echo $required_ratio.' ratio '.$aspect_ratio.'<br />';
        // Make fit and crop
        if ($required_ratio > $aspect_ratio) {
            //fit height and crop from the middle width.
            $x1 = (($max_height / $aspect_ratio) - $max_width) / 2;
            $x2 = $max_width + $x1;
            //echo $x1.' x '.$x2;
            return $this->fit_to_height($max_height)->crop($x1, 0, $x2, $max_height);
        } else {
            //fit width and crop from the middle height.
            $y1 = (($max_width * $aspect_ratio) - $max_height) / 2;
            $y2 = $max_height + $y1;
            //echo $y1.' y '.$y2;
            return $this->fit_to_width($max_width)->crop(0, $y1, $max_width, $y2);
        }
    ...
    public function overlay($overlay_file, $position = 'center', $opacity = 1, $x_offset = 0, $y_offset = 0) {
        ...
        // Load overlay image
        $overlay = $this->load($overlay_file); //new SimpleImage($overlay_file);
        // Convert opacity
        $opacity = $opacity * 100;
