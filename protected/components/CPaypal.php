 ...
 * @version 0.2.1
 */
class CPaypal extends CComponent {

    // API credentials
    public $username; //API username
    public $password; //API password
    public $signature; //API $signature
    // API configuration
    public $currency; //API currency code
    public $returnURL; //API return URL
    public $cancelURL; //API cancel URL
    public $environment; //'SANDBOX' or 'LIVE';
    private $debug = true;
    ...
    /**
     * Checks and sets API properties
     */
    public function init() {
        if ($this->environment !== 'LIVE' && $this->environment !== 'SANDBOX')
     ...
     */
    public function setExpressCheckout($returnURL = null, $cancelURL = null) {
        if (!isset($_GET['token'])) {
            is_null($returnURL) and $returnURL = $this->returnURL;
            is_null($cancelURL) and $cancelURL = $this->cancelURL;
    ...
    /**
     * Redirects to paypal with the response TOKEN returned by a successful setExpressCheckout action. 
     */
    public function expressCheckoutRedirect() {
    ...
    public function doExpressCheckoutPayment() {
        ...
        if ($this->execute()){
            $this->response['PAYERID'] = $d['PAYERID'];
            return $this->response;
        }
    ...
    public function execute() {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $this->endpoints[$this->environment]);
